...
void generatePipeRangeCode(Attribute* SS, const Attribute& start, const Attribute& end);
...
%token _FILTER _FIRST_N _FOR_EACH _LAST_N _SORT _RANGE _2PTS 
%token _INT _CHAR _BOOL _DOUBLE _FLOAT _STRING _CMD_OUT _CMD_IN _CMD_VAR _FUNCTION _BEGIN _END
%token _OPR_ATR _OPR_NOT _OPR_LESS_EQUAL _OPR_GREATER_EQUAL _OPR_COMP _OPR_AND _OPR_OR

%left _OPR_AND _OPR_OR
%nonassoc '<' '>' _OPR_LESS_EQUAL _OPR_GREATER_EQUAL _OPR_COMP
%left '+' '-'
%left '*' '/' '%'
...
ID : _ID { $$ = Attribute();
   ...
   | _ID '[' E ']' '[' E ']' { Type idType;
                               ...
                               $$ = Attribute();
                               $$.c = $3.c + $6.c +
                                      indexTemp1.v + " = " + toStr(idType.s) + " * " + $3.v + ";\n" + 
...
F : FUNC
  | ID
    { string id = trimId($1.v);
      if( searchVar( id, &$$.t ) ){
        $$.v = $1.v;
        $$.c = $1.c;
      }
      else
        error( "Variavel nao declarada: " + id );
    }
  | _CTE_INT
    {  $$.v = $1.v; 
       $$.t = Type( "int" ); }
...
RANGE : _RANGE '[' _CTE_INT '.''.' _CTE_INT ']' //{generatePipeRangeCode(&$$, $3.v, $6.v);}
      ;
          
FILTER : _FILTER '[' E ']'
       ;
             
FOR_EACH : _FOR_EACH '[' E ']'
         ;
...
ARR : _ID '[' ARR_SIZE ']' {generateArrayCode(&$$, $3);}
    | _ID '[' ARR_SIZE ']' '[' ARR_SIZE ']' { generateBidimensionalArrayCode(&$$, $3, $6); }
    ;
...
ARR_INIC : _OPR_ATR LIST //{ $$ = $2;}
         | /* producao vazia */
         ;         

LIST : '{' ITEM '}' //{$$ = Attribute(); $$.v = "{" + $2.v + "}";}
      ;

ITEM  : E //{$$.v = $1.v;}
      | ITEM ',' E //{$$.v = $1.v + $2.v + $3.v;}
      ;
...
void generatePipeRangeCode(Attribute* SS, const Attribute& start, const Attribute& end){
    Type type = Type("int");
    string var = generateTemp(type);
    Attribute initial, cond, step, varPipe, cmd = Attribute();
    *SS = Attribute();
    ...
    insertVarTS(tsl, varPipe.v, type);
    generateVarDeclaration(SS, varPipe, varPipe);
